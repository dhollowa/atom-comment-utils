...
import emit_cpp_comments_04 from './emit_cpp_comments_04'   // *  ...
import emit_cpp_comments_05 from './emit_cpp_comments_05'   // * ...
...
import parse_cpp_comments_04 from './parse_cpp_comments_04' // *  ...
import parse_cpp_comments_05 from './parse_cpp_comments_05' // * ...
...
export default class CommentUtils {
  ...
  cycleRange (marker, editor, scope) {
    ...
    if (!parsed) try {
      tree = parse_cpp_comments_04.parse(selection)
      parsed = true
      if (this.cycleCount == 0) {
        this.cycleCount = 3 // Prefer " *  ..." emitter.
      }
    } catch (e) {
    }

    if (!parsed) try {
      tree = parse_cpp_comments_05.parse(selection)
      parsed = true
      if (this.cycleCount == 0) {
        this.cycleCount = 4 // Prefer " * ..." emitter.
      }
    } catch (e) {
    }
    ...
    if (parsed) {
      ...
      emitterCount = 5
      newCount = 0
      if (this.cycleCount % emitterCount == 0) {
        result = emit_cpp_comments_01.emitCPPComments(prefix, tokens, eol, lineLength)
        newCount = 1
      } else if (this.cycleCount % emitterCount == 1) {
        result = emit_cpp_comments_02.emitCPPComments(prefix, tokens, eol, lineLength)
        newCount = 2
      } else  if (this.cycleCount % emitterCount == 2) {
        result = emit_cpp_comments_03.emitCPPComments(prefix, tokens, eol, lineLength)
        newCount = 3
      } else  if (this.cycleCount % emitterCount == 3) {
        result = emit_cpp_comments_04.emitCPPComments(prefix, tokens, eol, lineLength)
        newCount = 4
      } else  if (this.cycleCount % emitterCount == 4) {
        result = emit_cpp_comments_05.emitCPPComments(prefix, tokens, eol, lineLength)
        newCount = 5
      }
